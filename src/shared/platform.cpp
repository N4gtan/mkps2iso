...
{
    std::string result;
    const int count = WideCharToMultiByte(CP_UTF8, 0, str.data(), str.length(), nullptr, 0, nullptr, nullptr);
    if (count != 0)
    {
        result.resize(count);
...
{
    LARGE_INTEGER ll{.LowPart = ft.dwLowDateTime, .HighPart = static_cast<long>(ft.dwHighDateTime)};
...
{
#ifdef _WIN32
    return _wfopen(filename.c_str(), UTF8ToUTF16(mode).c_str());
#else
...
// Returns local UTC `tm` from UTC+0 `time_t`
...
// Returns UTC+0 `time_t` from local UTC `tm`
time_t MakeTime(struct tm *timeBuf)
{ // Windows mktime() can't handle timestamps prior to 1970
#ifdef _WIN32
    FILETIME localFT;
    SYSTEMTIME localST{
        .wYear   = static_cast<WORD>(timeBuf->tm_year + 1900),
        .wMonth  = static_cast<WORD>(timeBuf->tm_mon + 1),
        .wDay    = static_cast<WORD>(timeBuf->tm_mday),
        .wHour   = static_cast<WORD>(timeBuf->tm_hour),
        .wMinute = static_cast<WORD>(timeBuf->tm_min),
        .wSecond = static_cast<WORD>(timeBuf->tm_sec)};
...
{
    const time_t time = MakeTime(&timestamp);
