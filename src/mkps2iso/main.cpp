...
static void WriteLogLBA(const std::list<std::tuple<std::list<Entry>, uint32_t, uint32_t>> &layers)
{
    ...
    if (fp != nullptr)
    {
        ...
        fprintf(fp.get(), "Image file: \"%s\"\n", reinterpret_cast<const char *>(param::isoFile.generic_u8string().c_str()));
...
static bool BuildISO(xml::Reader &xml)
{
    ...
    if (!param::quietMode)
    {
        printf("Building ISO Image: \"%" PRFILESYSTEM_PATH "\"\n\n", param::isoFile.c_str());
...
int Main(int argc, char *argv[])
{
    ...
    for (char **args = argv + 1; *args != nullptr; args++)
    {
        ...
        {
            param::xmlFile = fs::path(reinterpret_cast<const char8_t *>(*args)).lexically_normal().lexically_proximate(fs::current_path());
        }
        else
        {
